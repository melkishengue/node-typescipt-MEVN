...
let databaseConfiguration: IDatabaseConfiguration = {
  host: process.env.DB_HOST_SEARCH,
  port: process.env.DB_PORT_SEARCH,
  username: process.env.DB_USER_SEARCH,
  password: process.env.DB_PASS_SEARCH,
  database: process.env.DB_NAME_SEARCH
}
...
MongooseDatabaseProvider.configure(databaseConfiguration).then(async (res) => {
  ...
  let server = new Server(+process.env.SERVER_PORT_SEARCH || 3002);
